...
# import roomPeerClient class from file ./roomPeerClient.py
from room import RoomPeerClient
            ...
            try:
                ...
                for s in readable:
                    ...
                    if s == self.tcpServerSocket:
                        ...
                        if self.isChatRequested == 0 and not self.inRoom:    
                    ...
                    # is used to communicate with a connected peer, then enters here
                        ...
                        if len(messageReceived) > 11 and messageReceived[:12] == "ROOM-MESSAGE":
                            self.inRoom = True
                            print(messageReceived[13:])
                            ...
                            elif s != self.connectedPeerSocket and (self.isChatRequested == 1 or self.inRoom):
                        ...
                        # if it is not an empty message, show this message to the user
                            ...
                            print(f"{self.chattingClientName} :  {messageReceived}")
                        ...
                        elif len(messageReceived) > 11 and messageReceived[:12] == "ROOM-MESSAGE" and ":q" in messageReceived:
                            self.isChatRequested = 0
                            inputs.clear()
...
class PeerClient(threading.Thread):
    ...
    def createClientSockets(self, servers):
        for ip, port in servers:
            self.tcpClientSocket.append(socket.socket(socket.AF_INET, socket.SOCK_STREAM))
            self.tcpClientSocket[-1].connect((ip, port))
            self.tcpClientSocket[-1].setblocking(0)
    
    def sendMessageToAll(self, message):
        for socket in self.tcpClientSocket:
            socket.send(message)
        
    # main method of the peer client thread
    def run(self):
        print("Peer client started...")
        print(f"Peer client is connecting to {self.ipToConnect}:{self.portToConnect}...")
...
class peerMain:
    ...
    def __init__(self):
        ...
        while choice != "3":
            ...
            # to enter the username of the user that is wanted to be chatted
                ...
                servers = self.joinRoom(roomId)
                if servers == None: 
                    continue
                print("Enter :q to leave the room at any time ...")
                self.roomPeerClient = RoomPeerClient(servers, self.loginCredentials[0], self.peerServer, roomId, self.tcpClientSocket)
                self.roomPeerClient.start()
                self.roomPeerClient.join()
                ...
                if not self.isOnline:
                    ...
                    continue
                roomId = input("Enter the room ID: ")
                createStatus = self.createRoom(roomId)
                print(createStatus)

            elif choice == "8":
                availabeRooms = self.getAvailableRooms()
    ...
    def getAvailableRooms(self):
        ...
        logging.info(f"Recieved from {self.registryName} -> {response}")
        
        response_splitted = response.strip().split()

        if response_splitted[0] == "no-available-rooms":
            print ("No available rooms")
            return
        available_rooms = response_splitted[1].strip().split(',')
    ...
    def joinRoom(self, room_id):
        ...
        logging.info(f"Recieved from {self.registryName} -> {response}")        
        # print(response)
        if response.split()[0] == "join-room-fail":
            print("Failed to join room, probably room does not exist")
            return None
        # join-room-success qw ss,192.168.1.15,50940 zz,192.168.1.15,47576
        servers = {}
        for server in response.split()[2:]:
